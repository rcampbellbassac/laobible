<template>
  <div class="about">
    <h1>About Us</h1>

    <p>
      The last iteration of the Lao Books series apps,
      crafted from sources like
      <a href="http://laobible.net/" target="_blank">
        LaoBible.net
        <i class="material-icons">
          open_in_new
        </i>
      </a> and the
      <a href="http://www.adventist.la/" target="_blank">
        Lao Seventh-day Adventist Church
        <i class="material-icons">
          open_in_new
        </i>
      </a>,
      now come as sleek, minimal progressive web apps. They support
      offline use. They are easy to navigate. They are simple.
      And, they come as a result of technological and accessibility advances in
      Lao. Now that Lao can be easily created, accessed, and shared online -
      the first Android apps to ever published in Lao without using
      'invisible spaces', are now here in their latest form. They will remain
      here free to access and use.
    </p>

    <p><em>Specifically for the Lao Bible...</em></p>

    <p><strong>There are 2 options:</strong></p>

    <p>
      <ul class="list-group">
        <li class="list-group-item">
          <a href="https://laobible.sea-sda.org">The LaoBible Web App</a>
        </li>
        <li class="list-group-item">
          <a href="https://play.google.com/store/apps/details?id=org.ubs.lao.rlv"
          target="_blank">
          The Lao Revised Version
          <i class="material-icons">
            open_in_new
          </i>
        </a>
      </li>
    </ul>
    </p>

    <blockquote class="blockquote text-center">
      <p class="mb-0">
        If you lose Heaven, you lose everything;
        if you gain Heaven, you gain everything.
      </p>
      <footer class="blockquote-footer">
        Ellen Gould White in
        <cite title="Source Title">Our Father Cares</cite>
      </footer>
    </blockquote>

    <p>
      <small>
        Photo of the Bible by David Ball: www.davidball.net <br>
        Licensed <a href="https://creativecommons.org/licenses/by/2.5/deed.en">CC BY 2.5</a>
      </small>
    </p>
  </div>
</template>

<style>
  .material-icons {
    font-size: 1rem;
  }
</style>
